 ...
 */
        ...
        window.twSDK = {
            ...
            buildingPoints: {
                main: [
                    10, 2, 2, 3, 4, 4, 5, 6, 7, 9, 10, 12, 15, 18, 21, 26, 31, 37,
                    44, 53, 64, 77, 92, 110, 133, 159, 191, 229, 274, 330,
                ],
                barracks: [
                    16, 3, 4, 5, 5, 7, 8, 9, 12, 14, 16, 20, 24, 28, 34, 42, 49, 59,
                    71, 85, 102, 123, 147, 177, 212,
                ],
                stable: [
                    20, 4, 5, 6, 6, 9, 10, 12, 14, 17, 21, 25, 29, 36, 43, 51, 62,
                    74, 88, 107,
                ],
                ...
                church_f: [10],
                watchtower: [
                    42, 8, 10, 13, 14, 18, 20, 25, 31, 36, 43, 52, 62, 75, 90, 108,
                    130, 155, 186, 224,
                ],
                snob: [512],
                smith: [
                    19, 4, 4, 6, 6, 8, 10, 11, 14, 16, 20, 23, 28, 34, 41, 49, 58,
                    71, 84, 101,
                ],
                place: [0],
                statue: [24],
                market: [
                    10, 2, 2, 3, 4, 4, 5, 6, 7, 9, 10, 12, 15, 18, 21, 26, 31, 37,
                    44, 53, 64, 77, 92, 110, 133,
                ],
                wood: [
                    6, 1, 2, 1, 2, 3, 3, 3, 5, 5, 6, 8, 8, 11, 13, 15, 19, 22, 27,
                    32, 38, 46, 55, 66, 80, 95, 115, 137, 165, 198,
                ],
                stone: [
                    6, 1, 2, 1, 2, 3, 3, 3, 5, 5, 6, 8, 8, 11, 13, 15, 19, 22, 27,
                    32, 38, 46, 55, 66, 80, 95, 115, 137, 165, 198,
                ...
                farm: [
                    5, 1, 1, 2, 1, 2, 3, 3, 3, 5, 5, 6, 8, 8, 11, 13, 15, 19, 22,
                    27, 32, 38, 46, 55, 66, 80, 95, 115, 137, 165,
                ],
                storage: [
                    6, 1, 2, 1, 2, 3, 3, 3, 5, 5, 6, 8, 8, 11, 13, 15, 19, 22, 27,
                    32, 38, 46, 55, 66, 80, 95, 115, 137, 165, 198,
                ...
                wall: [
                    8, 2, 2, 2, 3, 3, 4, 5, 5, 7, 9, 9, 12, 15, 17, 20, 25, 29, 36,
                    43,
                ],
            },
            unitsFarmSpace: {
            ...
            _registerScript: function () {
                if (this.enableCountApi) {
                    ...
                    const scriptInfo = this.scriptInfo();
                    jQuery.getJSON(
                        `https://twscripts.dev/count/?script=$`,
                        ({ count }) => {
                            console.debug(
            ...
            getBuildingsInfo: async function () {
                ...
                if (LAST_UPDATED_TIME !== null) {
                    if (
                        Date.parse(new Date()) >=
                        LAST_UPDATED_TIME + TIME_INTERVAL
                    ) {
            ...
            getDestinationCoordinates: function (
                config,
                tribes,
                players,
                villages
                ...
                const tribePlayers = twSDK.getTribeMembersById(
                    chosenTribeIds,
                    players
                );
                ...
                if (enable20To1Limit) {
                    ...
                    const playersNotBiggerThen20Times =
                        uniquePlayersListArray.filter((player) => {
                            return (
                                parseInt(player[4]) <=
                        ...
                        coordinatesArray = chosenContinentsArray
                            .map((continent) => {
                                if (continent.length && $.isNumeric(continent)) {
                                    return [
                                        ...filteredVillagesByContinent[continent],
                                    ];
                                } else {
                                    return;
                                }
                        ...
                        if (distribution > 1) {
                            players.forEach((player) => {
                                if (
                                    ...
                                    const flattenedPlayerVillagesArray =
                                        makeRepeated(playerVillages, distribution);
            ...
            getFilteredVillagesByContinent: function (
                ...
                coords.forEach((coord) => {
                    continents.forEach((continent) => {
                        let currentVillageContinent =
                            twSDK.getContinentByCoord(coord);
            ...
            getGameFeatures: function () {
                const { Premium, FarmAssistent, AccountManager } =
                    game_data.features;
            ...
            getWorldConfig: async function () {
                ...
                if (LAST_UPDATED_TIME !== null) {
                    if (
                        ...
                        worldConfig = JSON.parse(
                            localStorage.getItem('world_config')
                        );
                    }
                } else {
                    const response = await jQuery.ajax({
            ...
            renderFixedWidget: function (
                ...
                if (jQuery(`#${id}`).length < 1) {
                    if (mobiledevice) {
                        ...
                        jQuery(`#${id} .custom-close-button`).on(
                            'click',
                            function (e) {
                                e.preventDefault();
                                jQuery(`#${id}`).remove();
